...
class AdminRecordRechargePage extends StatefulWidget {
  final Function onSwitchParentType;       /// 切换上级状态
  AdminRecordRechargePage({ @required this.onSwitchParentType });
  
  @override
  _AdminRecordRechargePage createState() => _AdminRecordRechargePage();
}

class _AdminRecordRechargePage extends State<AdminRecordRechargePage> {
  DateTime timeFilter;
  List<dynamic> arrRecordData = [];
  RefreshController _refreshController = RefreshController(initialRefresh: false);
  StreamSubscription streamSubscription;
  String _listType = "RecordRecharge";
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          AdminHeaderPage(title: "充值记录",
            onBack: () { widget.onSwitchParentType(630); },
            onFilter: () {
  ...
  void getRecordList() {
    HttpUtil.getInstance().postJson(API.getRecordPayList, params: {
      "time": timeFilter == null ? null : DateFormat("yyyy-MM-dd HH:mm").format(timeFilter)
    }).then((response) {
      if (response == null) {
      ...
      setRecordList(json["data"]);
    });
  }

  void setRecordList(arr) {
    setState(() {
      arrRecordData = arr;
    });
  }
