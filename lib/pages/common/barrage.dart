...
class Barrage extends StatefulWidget {
  Barrage(this.barrageController, { Key key })
      : super(key: key);
...
class _PlayPauseState extends ValueNotifier<bool> {
  _PlayPauseState(this.controller)
      : super(controller.value.isPlaying) {
    _listener = () {
...
class BarrageChannel extends StatefulWidget {
  const BarrageChannel(this.controller, { Key key })
      : super(key: key);
...
class _BarrageChannelState extends State<BarrageChannel> {
  ...
  List<Widget> get _barrages =>
      _controller.value.values.map((e) => Barrage(e))
          .toList();
...
class BarrageWallController {
  BarrageWallController.options(List<ChannelOptions> options)
      : assert(options != null),
        _channels = options.map((e) => ChannelController(e))
